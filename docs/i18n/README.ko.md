...
## ✅ 지금 바로 온라인에서 사용해 보세요 — 설치도, API 키도 필요 없습니다

이 skill의 무료 호스팅 **리뷰 전용** 버전이
**[zoelin.dev/tools/paper-revision](https://zoelin.dev/tools/paper-revision)**에서
운영되고 있습니다. PDF나 LaTeX 소스를 업로드하고 대상 학회를 선택하면, 브라우저에서 바로
오리엔테이션 브리핑과 심각도순 발견 목록을 받아 볼 수 있습니다——무료 호스팅 모델이
기본 포함되어 있고, 더 높은 품질의 출력을 원하면 자신의 API 키를 사용할 수도 있습니다.
파일을 편집하거나 저장하지 않습니다. **전체 개정** 모드(Claude가 실제로 `.tex`를 다시 쓰는
방식)를 사용하려면 아래 설명대로 Claude Code에서 이 skill을 직접 사용하세요.
...
2. **리뷰 전용(편집 없음).** Claude가 초안을 읽고 *개요와 코멘트만* 돌려줍니다——연구 주제/범위,
   핵심 기여, 솔직한 참신성 평가, 문제점, 구체적 제안——파일은 전혀 바꾸지 않습니다.
   [호스팅되는 웹 앱](https://zoelin.dev/tools/paper-revision)이 노출하는 모드가 바로 이것입니다.
