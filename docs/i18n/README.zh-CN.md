...
[English](../../README.md) | **中文** | [日本語](README.ja.md) | [한국어](README.ko.md) | [Español](README.es.md) | [Français](README.fr.md) | [Deutsch](README.de.md) | [Português](README.pt.md)
<!-- 翻译由社区维护；英文为准，可能更新更快。 -->
...
## ✅ 立即在线试用——无需安装，无需 API key

这个 skill 有一个免费托管的**仅审阅**版本，运行在
**[zoelin.dev/tools/paper-revision](https://zoelin.dev/tools/paper-revision)**：
上传 PDF 或 LaTeX 源文件，选择目标会议，就能在浏览器里直接拿到定位简报和按严重程度
排序的发现清单——内置免费托管模型，也可以用你自己的 API key 换取更高质量的输出。
它不会编辑或存储你的文件；如果需要**完整修订**模式（让 Claude 直接改写你的 `.tex`），
请按下文说明直接用 Claude Code 使用这个 skill。
...
2. **仅审阅（不编辑）。** Claude 读草稿，只返回*总览与评论*——研究主题/范围、核心贡献、
   坦诚的新颖性评估、问题、以及具体建议——不改动任何文件。这正是
   [托管的网页应用](https://zoelin.dev/tools/paper-revision)所暴露的模式。

两种模式都可以用**任意语言**工作：用中文、西班牙语、阿拉伯语或任何其他语言提问并接收
总览、评论和摘要。只有*对话*跟随你的语言——稿件本身仍保持会议要求的语言（通常是英语）。

详细走查见[用户手册（中文）](USER-GUIDE.zh-CN.md)（English: [`docs/USER-GUIDE.md`](../USER-GUIDE.md)）。
