...
> **ステータス：** skill と合成サンプルドラフトは今すぐ使えます。ホスト型の読み取り専用
> PDF レビュアー（Vercel / GitHub Pages）は[ロードマップ](#ロードマップ)にあります。
...
2. **レビューのみ（編集なし）。** Claude はドラフトを読み、*概観とコメントのみ*を返します——
   ...
   変更しません。計画中の Web アプリが公開するのはこのモードです。
...
ホスト型の**読み取り専用 PDF レビュアー**：ユーザーが PDF ドラフトをアップロードすると、概観と
コメント——研究テーマと範囲、扱う問題、中核的貢献、率直な新規性評価、改善提案——を受け取ります
（**LaTeX 編集は一切なし**）。上記のレビューのみモードの上に載せる小さな Web フロントエンド
（Vercel または GitHub Pages）として計画中です。同じリポジトリで追跡する別タスクで、解析ロジック
は既に skill 内にあります。
